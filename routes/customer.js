...
router.post("/bookings", auth, (req, res) => {
  const { room, price_per_night, checkin, checkout, guests, special_request } =
    req.body;
...
router.post("/orders", auth, (req, res) => {
  ...
  db.orders.insert(doc, () =>
    res.json({ ok: true, msg: "Order placed! Estimated 20-30 minutes." }),
  );
});
...
router.post("/requests", auth, (req, res) => {
  ...
  if (!detail || !detail.trim())
    return res.json({ ok: false, msg: "Please describe your request" });
  const u = req.session.user;
  const doc = {
    id: "REQ" + Date.now(),
    user_id: u.id,
